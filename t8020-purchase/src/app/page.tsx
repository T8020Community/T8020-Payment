...
import { ThirdwebProvider, ConnectButton, PayEmbed, useActiveAccount, useReadContract } from "thirdweb/react";
import { createThirdwebClient, getContract, readContract, sendTransaction } from "thirdweb";
...
function Purchase() {
  const [currency, setCurrency] = useState<"POL" | "MATIC" | "USDC">("POL");
  ...
  const payEmbedRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const fetchClaimConditions = async (attempt = 1) => {
      try {
        ...
        const conditions = await readContract({
          contract,
        ...
        if (attempt < 3) {
          ...
          setError(
            `Failed to load claim conditions: ${errorMsg}. Run the setup script to initialize tokenId 0 and set claim conditions.`
          );
        }
      }
    };
  ...
  const handlePayButtonClick = () => {
    ...
    setTimeout(() => {
      if (payEmbedRef.current) {
        payEmbedRef.current.scrollIntoView({ behavior: "smooth" });
      }
    }, 100);
  };
  ...
  return (
    <div style={{ padding: "20px", textAlign: "center" }}>
      <h1>T8020 Orlando Membership Token Purchase</h1>
      {/* ConnectButton */}
      <ConnectButton client={client} chain={polygon} />
      {/* Custom Button for PayEmbed */}
      <button
      ...
      >
        Purchase T8020ORL Membership Tokens
      </button>
      {isPayEmbedOpen && claimTransaction && (
        <div ref={payEmbedRef}>
          <PayEmbed
            client={client}
            payOptions={{
              ...
              buyWithFiat: { testMode: true, prefillSource: { currency: "USD" } },
            }}
          />
        </div>
      )}
      <div style={{ marginTop: "20px" }}>
        <p>Price: {TOKEN_PRICE_POL} {currency}</p>
        <div style={{ marginBottom: "10px" }}>
          <label>Currency: </label>
          <select
            value={currency}
            onChange={(e) => setCurrency(e.target.value as "POL" | "MATIC" | "USDC")}
            style={{ marginLeft: "5px" }}
          >
            <option value="POL">POL (Native)</option>
            <option value="MATIC">MATIC (ERC-20)</option>
            <option value="USDC">USDC (Fiat/Card)</option>
          </select>
        </div>
        {claimConditions ? (
          <p>
            Contract Price: {ethers.utils.formatEther(claimConditions.pricePerToken)} {claimConditions.currency === ethers.constants.AddressZero ? "POL" : claimConditions.currency === MATIC_ERC20_ADDRESS ? "MATIC" : "USDC"}
          </p>
        ) : (
          <p>No claim conditions loaded. Run setup script or check thirdweb dashboard.</p>
        )}
        {error && <p style={{ color: "red" }}>{error}</p>}
        {retryCount > 0 && <p>Retrying claim conditions load ({retryCount}/3)...</p>}
        {/* Fallback Custom Buy Button */}
        <button
        ...
        >
          Purchase T8020ORL Membership Tokens (Fallback)
        </button>
      </div>
    </div>
  );
